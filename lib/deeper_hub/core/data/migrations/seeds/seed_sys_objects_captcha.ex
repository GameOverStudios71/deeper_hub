defmodule DeeperHub.Core.Data.Migrations.Seeds.SysObjectsCaptchaSeed do
  ...
  """
  def run do
    Logger.info("Inserindo registros na tabela sys_objects_captcha...", module: __MODULE__)
    ...
    Repo.execute("INSERT OR REPLACE INTO sys_objects_captcha (id, object, title, override_class_name, override_class_file) VALUES (?, ?, ?, ?, ?)", [3, "sys_hcaptcha", "hCaptcha", "BxTemplCaptchaHCaptcha", ""])
      Logger.info("Registros inseridos com sucesso na tabela sys_objects_captcha!", module: __MODULE__)
    rescue
      error ->
        Logger.error("Erro ao inserir registros na tabela sys_objects_captcha: #{inspect(error)}", module: __MODULE__)
