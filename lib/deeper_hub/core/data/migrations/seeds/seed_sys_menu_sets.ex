defmodule DeeperHub.Core.Data.Migrations.Seeds.SysMenuSetsSeed do
  ...
  """
  def run do
    Logger.info("Inserindo registros na tabela sys_menu_sets...", module: __MODULE__)
    ...
    Repo.execute("INSERT OR REPLACE INTO sys_menu_sets (set_name, module, title, deletable) VALUES (?, ?, ?, ?)", ["bx_persons_menu_manage_tools", "bx_persons", "_bx_persons_menu_set_title_manage_tools", 0])
      Logger.info("Registros inseridos com sucesso na tabela sys_menu_sets!", module: __MODULE__)
    rescue
      error ->
        Logger.error("Erro ao inserir registros na tabela sys_menu_sets: #{inspect(error)}", module: __MODULE__)
