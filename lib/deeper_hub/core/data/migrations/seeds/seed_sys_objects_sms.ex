defmodule DeeperHub.Core.Data.Migrations.Seeds.SysObjectsSmsSeed do
  ...
  """
  def run do
    Logger.info("Inserindo registros na tabela sys_objects_sms...", module: __MODULE__)
    ...
    Repo.execute("INSERT OR REPLACE INTO sys_objects_sms (id, object, title, override_class_name, override_class_file) VALUES (?, ?, ?, ?, ?)", [2, "sys_smsru", "SMS.RU", "BxDolSmsSmsRu", ""])
      Logger.info("Registros inseridos com sucesso na tabela sys_objects_sms!", module: __MODULE__)
    rescue
      error ->
        Logger.error("Erro ao inserir registros na tabela sys_objects_sms: #{inspect(error)}", module: __MODULE__)
