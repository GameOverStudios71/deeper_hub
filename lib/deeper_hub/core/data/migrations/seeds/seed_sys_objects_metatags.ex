defmodule DeeperHub.Core.Data.Migrations.Seeds.SysObjectsMetatagsSeed do
  ...
  @seed_name "sys_objects_metatags_seed"
  ...
  def run do
    if seed_already_executed?() do
      Logger.info("Seed para sys_objects_metatags já foi executado anteriormente. Pulando...", module: __MODULE__)
      :already_executed
    else
      Logger.info("Executando seed para a tabela sys_objects_metatags...", module: __MODULE__)
    ...
    Repo.execute("INSERT OR REPLACE INTO sys_objects_metatags (id, object, module, table_keywords, table_locations, table_mentions, override_class_name, override_class_file) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [2, "bx_persons", "bx_persons", "bx_persons_meta_keywords", "bx_persons_meta_locations", "bx_persons_meta_mentions", "", ""])
        ...
        mark_seed_executed()
        Logger.info("Seed para sys_objects_metatags executado com sucesso!", module: __MODULE__)
        :ok
      rescue
        error ->
          error_message = "#{Exception.message(error)}"
          Logger.error("Erro ao executar seed para sys_objects_metatags: #{error_message}", module: __MODULE__)
          {:error, error}
      end
    end
  end
  ...
  """
  def reset do
    Logger.info("Resetando seed para sys_objects_metatags...", module: __MODULE__)
    seed_file = Path.join(@seeds_dir, @seed_name)
    File.rm(seed_file)
    Logger.info("Seed sys_objects_metatags será re-executado na próxima inicialização.", module: __MODULE__)
  end
