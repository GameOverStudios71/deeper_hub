defmodule DeeperHub.Core.Data.Migrations.SysTranscoderVideosFiles do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao criar tabela de sys_transcoder_videos_files: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_transcoder_videos_files: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao remover tabela de sys_transcoder_videos_files: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_transcoder_videos_files: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
end
