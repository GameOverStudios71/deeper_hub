defmodule DeeperHub.Core.Data.Migrations.BxTimelineMetaLocations do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao criar tabela de bx_timeline_meta_locations: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_timeline_meta_locations: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao remover tabela de bx_timeline_meta_locations: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_timeline_meta_locations: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
end
