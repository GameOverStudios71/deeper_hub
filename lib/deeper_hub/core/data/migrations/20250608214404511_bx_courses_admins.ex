defmodule DeeperHub.Core.Data.Migrations.BxCoursesAdmins do
  ...
  def up do
    ...
    expired INTEGER NOT NULL DEFAULT 0,
      UNIQUE (group_profile_id, fan_id)
    );
    """
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao criar tabela de bx_courses_admins: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_courses_admins: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao remover tabela de bx_courses_admins: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_courses_admins: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
end
