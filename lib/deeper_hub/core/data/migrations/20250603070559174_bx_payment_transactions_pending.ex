defmodule DeeperHub.Core.Data.Migrations.BxPaymentTransactionsPending do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao criar tabela de bx_payment_transactions_pending: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_payment_transactions_pending: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao remover tabela de bx_payment_transactions_pending: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_payment_transactions_pending: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
end
