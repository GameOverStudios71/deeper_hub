defmodule DeeperHub.Core.Data.Migrations.BxGroupsInvites do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao criar tabela de bx_groups_invites: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_groups_invites: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, %Exqlite.Error{message: message}} ->
        Logger.error("Falha ao remover tabela de bx_groups_invites: #{message}", module: __MODULE__)
        {:error, message}
        
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_groups_invites: #{inspect(reason)}", module: __MODULE__)
        {:error, reason}
    end
  end
end
